...
class Game:
    ...
    def TestLevel(self):
        ...
        def f(z):
            a = (abs(z)**1.1)/2.5; return a - (a / 2) # (math.log(z + 2) ** .9) * 3
...
class Engine:
    ...
    def _createGlobals(self):
        ...
        def screenpos(x, y):
            newpos = vec3((x + screen_width/2), (-y + screen_height/2), 1e-9)
    ...
    class Projection:
        ...
        def projectTriangle(tri, Object):
            ...
            for ctri in projectedTris:
                clippedTris, nodraw, clipped = self.clipTriangleAgainstPlane(
                    vector(vec3(0, screen_height/2, 0), vec3(0, -1, 0)), [ctri])
                if nodraw: continue
                clippedTris, nodraw, clipped = self.clipTriangleAgainstPlane(
                    vector(vec3(0, -screen_height/2, 0), vec3(0, 1, 0)), clippedTris)
                if nodraw: continue
                clippedTris, nodraw, clipped = self.clipTriangleAgainstPlane(
                    vector(vec3(screen_width/2, 0, 0), vec3(-1, 0, 0)), clippedTris)
                if nodraw: continue
                clippedTris, nodraw, clipped = self.clipTriangleAgainstPlane(
                    vector(vec3(-screen_width/2, 0, 0), vec3(1, 0, 0)), clippedTris)
                if nodraw: continue
    ...
    class RenderClass:
        def __init__(self):
            ...
            self.infoFont = pygame.freetype.SysFont('Courier New', 18, bold = True)
        ...
        def renderScene(self):
            ...
            canvas.fill(engine.World.SkyColor)
            ...
            pygame.display.flip()  # Update canvas with rasterized scene
            ...
            pygame.display.set_caption("3D Engine " + str(round(1/(self.frametime + 1e-9), 1)))
        ...
        def _drawVector(self, rvec):
            ...
            pygame.draw.line(
                canvas,
                color(rvec.color.x, rvec.color.y, rvec.color.z),
                screenpos(vec.vec0.x, vec.vec0.y),
                screenpos(vec.vec1.x, vec.vec1.y),
                width = self.lineSize
            )

        def _drawTriangle(self, rtri):
            tri, c, w = rtri.tri, rtri.color, 0
            points = [(screenpos(tri.p1.x, tri.p1.y)), (screenpos(tri.p2.x, tri.p2.y)), (screenpos(tri.p3.x, tri.p3.y))]
            if self.Wireframe: w = self.lineSize
            pygame.draw.polygon(canvas, color(c.tuple()), points, w)
        ...
        def _drawInfo(self):
            ...
            text, rect = self.infoFont.render(roundedPosition.__str__(), (255, 255, 255), (0, 0, 0, 0))
            canvas.blit(text, screenpos(380, 340))
            ...
            text, rect = self.infoFont.render(roundedPosition.__str__(), (255, 255, 255), (0, 0, 0, 0))
            canvas.blit(text, screenpos(380, 310))

            # Triangle Count
            triangles = str(self.triangleCounter)
            text, rect = self.infoFont.render("#tri: " + triangles, (255, 255, 255), (0, 0, 0, 0))
            canvas.blit(text, screenpos(-620, 340))
